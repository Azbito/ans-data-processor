...
def controller():
    ...
    with patch("controllers.csv.R2Service", return_value=mock_r2), patch(
        "controllers.csv.ZIPService", return_value=mock_zip
    ), patch("controllers.csv.CSVService", return_value=mock_csv), patch(
        "controllers.csv.PDFService", return_value=mock_pdf
    ):
...
def test_download_unzipped_table_success(controller):
    ...
    extracted_file = "/tmp/raw/tables.csv"
    extract_dir = "/tmp/extracted"
    temp_zip_path = "/tmp/tables.zip"
    r2_path = "csv/raw/tables.csv"
    final_url = "https://r2.example.com/csv/raw/tables.csv"
    ...
    assert isinstance(result, JSONResponse)
    assert result.status_code == 200
    assert result.body == b'{"url":"https://r2.example.com/csv/raw/tables.csv"}'

    controller.pdf_service.extract_tables.assert_called_once()
    extract_tables_args, extract_tables_kwargs = (
        controller.pdf_service.extract_tables.call_args
    )
    ...
    assert args[1] == "csv/raw/tables.csv"
    assert args[2] == "csv"

    controller.r2_service.get_file.assert_called_once_with("csv/raw/tables.csv")
